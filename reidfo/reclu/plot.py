...
def _sort_dates(
    regime_changes: Dict[str, List[pd.Timestamp]],
...
def plot_cluster_line(
    ...
    :param regime_changes: Dict with keys as time series names and values as lists of regime change dates.
    :param colours: Optional list of colours for each time series.
    :param monochrome: If True, use the same color for all series.
    """
    all_dates, _, _, colours = _sort_dates(regime_changes, colours, monochrome)
...
def plot_dendrogram(distance_matrix: Union[pd.DataFrame, np.ndarray], ylabel: str = "Distance") -> None:
    """
    Plot a hierarchical clustering dendrogram from a pairwise distance matrix.

    :param distance_matrix: Square, symmetric matrix of pairwise distances.
    :param ylabel: Label for the y-axis.
    """
    linkage_matrix = linkage(squareform(distance_matrix), method="average")
    plt.figure()
    labels = distance_matrix.index.tolist() if isinstance(distance_matrix, pd.DataFrame) else None
